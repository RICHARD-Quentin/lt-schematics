...
class ModuleDeclarator {
  constructor(
    imports = new module_import_declarator_1.ModuleImportDeclarator(),
    metadata = new module_metadata_declarator_1.ModuleMetadataDeclarator(),
  ) {
  ...
  computeSymbol(options) {
    ...
    if (options.className) {
      ...
      target.symbol = (0, strings_1.classify)(options.name).concat(
        (0, strings_1.capitalize)(options.type),
      )
    } else {
